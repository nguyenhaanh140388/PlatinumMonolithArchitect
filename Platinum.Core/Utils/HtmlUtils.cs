...
namespace Anhny010920.Core.Utilities
{
    public static class HtmlUtils
    {
        ...
        /// <summary>
        /// Sanitizes the specified t.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="t">The t.</param>
        /// <returns>Result.</returns>
        public static T Sanitize<T>(T t)
        {
            PropertyInfo[] props = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance);

            for (int i = 0; i < props.Length; i++)
            {
                if (typeof(string).IsAssignableFrom(props[i].PropertyType))
                {
                    object value = props[i].GetValue(t, null);
                    if (value != null)
                    {
                        value = EncodeHtml(value.ToString());
                        props[i].SetValue(t, Convert.ChangeType(value, props[i].PropertyType), null);
                    }
                }
            }
        ...
        private static string DefaultHtmlSanitizeTagBlackList { get; } = "script|iframe|object|embed|form";

        private static readonly Regex _RegExScript = new Regex($@"(<({DefaultHtmlSanitizeTagBlackList})\b[^<]*(?:(?!<\/({DefaultHtmlSanitizeTagBlackList}))<[^<]*)*<\/({DefaultHtmlSanitizeTagBlackList})>)",
        RegexOptions.IgnoreCase | RegexOptions.Multiline);

        // strip javascript: and unicode representation of javascript:
        // href='javascript:alert(\"gotcha\")'
        // href='&#106;&#97;&#118;&#97;&#115;&#99;&#114;&#105;&#112;&#116;:alert(\"gotcha\");'
        private static readonly Regex _RegExJavaScriptHref = new Regex(
            @"<[^>]*?\s(href|src|dynsrc|lowsrc)=.{0,20}((javascript:)|(&#)).*?>",
            RegexOptions.IgnoreCase | RegexOptions.Singleline);
        private static readonly Regex _RegExOnEventAttributes = new Regex(
            @"<[^>]*?\s(on[^\s\\]{0,20}=([""].*?[""]|['].*?['])).*?(>|\/>)",
            RegexOptions.IgnoreCase | RegexOptions.Singleline);
